...
void fat32_name_to_83(const char *name, char *name83)
{
    ...
    if (dot) {
        for (int i = 0; i < 3 && i < ext_len; i++) {
            name83[8 + i] = (dot[1 + i] >= 'a' && dot[1 + i] <= 'z') ?
                            (dot[1 + i] - 'a' + 'A') : dot[1 + i];
        }
    }
}
...
void fat32_83_to_name(const char *name83, char *name)
{
    ...
    for (i = 0; i < 8 && name83[i] != ' '; i++) {
        name[j++] = (name83[i] >= 'A' && name83[i] <= 'Z') ?
                    (name83[i] - 'A' + 'a') : name83[i];
    }
    ...
    while (cluster >= 2 && cluster < FAT32_EOC_MIN) {
        ...
        for (int i = 0; i < num_entries; i++) {
            ...
            if (strcmp(entry_name, name) == 0) {
                ...
                node->mode = (entries[i].attr & FAT32_ATTR_DIRECTORY) ? 
...
int fat32_readdir(vfs_file_t *file, struct dirent *dirent, size_t count)
{
    ...
    while (cluster >= 2 && cluster < FAT32_EOC_MIN && entries_read < (int)count) {
        ...
        for (int i = 0; i < num_entries && entries_read < (int)count; i++) {
            ...
            dirent[entries_read].d_type = (entries[i].attr & FAT32_ATTR_DIRECTORY) ?
...
vfs_superblock_t* fat32_mount(const char *dev, uint32_t flags)
{
    ...
    if (blkdev_read(blkdev, 0, 1, (uint8_t*)&fs->boot) != 0) {
    ...
    fs->fat_cache_size = fs->total_clusters + 2;
    fs->fat_cache = (uint32_t*)malloc(fs->fat_cache_size * sizeof(uint32_t));
    if (fs->fat_cache) {
        printf("[FAT32] Caching FAT table (%d KB)...\n", 
               (fs->fat_cache_size * 4) / 1024);
        
        uint32_t fat_bytes = fs->boot.fat_size_32 * fs->boot.bytes_per_sector;
        uint8_t *fat_buf = (uint8_t*)malloc(fat_bytes);
        if (fat_buf) {
            for (uint32_t i = 0; i < fs->boot.fat_size_32; i++) {
                blkdev_read(blkdev, fs->fat_offset + i, 1,
                           fat_buf + (i * fs->boot.bytes_per_sector));
            }
            
            uint32_t *fat_table = (uint32_t*)fat_buf;
            for (uint32_t i = 0; i < fs->fat_cache_size; i++) {
                fs->fat_cache[i] = fat_table[i] & 0x0FFFFFFF;
            }
    ...
    if (!sb) {
        if (fs->fat_cache) free(fs->fat_cache);
        free(fs);
        return NULL;
    }
...
void fat32_unmount(vfs_superblock_t *sb)
{
    ...
    if (fs) {
        if (fs->fat_cache && fs->fat_dirty) {
            printf("[FAT32] Writing back FAT cache...\n");
        }
